...
class AboutRegularExpressions < Neo::Koan
  ...
  def test_slash_a_anchors_to_the_start_of_the_string
    ...
    assert_equal nil, "start end"[/\Aend/] #bc it's not at the start of the string
  end
  ...
  def test_caret_anchors_to_the_start_of_lines
    assert_equal "2", "num 42\n2 lines"[/^\d+/] #Matches at the beggining of a line
  end
  ...
  def test_sub_is_like_find_and_replace
    assert_equal "one t-three", "one two-three".sub(/(t\w*)/) { $1[0, 1] } #first word that starts with a 't' { $1[0, 1] } $1 = two $1 = t
  end
